...
== Lab 5: Stream Data to AWS Redshift

We can use the JDBC Sink Connector to stream changes from our Confluent Cloud topics to AWS Redshift.

image::./images/7_cloud_services.png[]

Run the following from the command line to create the JDBC sink Connector instance, this connector will sink events from Confluent Cloud to AWS Redshift.
...
Verify that the JDBC sink connector is in a `RUNNING` state.

[source,subs="attributes"]
----
curl -s localhost:18084/connectors/{dc}_redshift_sink/status | jq
----
...
Depending on who's hosting the workshop, you may or may not have access to the AWS account where the Redshift cluster is hosted. Ask your instructor to verify that your data is arriving in AWS Redshift.


== Lab 6: Creating a ksqlDB Application
...
== Lab 7: Querying Streams with ksqlDB
...
== Lab 9: Joining Steams & Tables with ksqlDB
...
== Lab 10: Stream Sales & Purchases to S3
...
Depending on who's hosting the workshop, you may or may not have access to the AWS account where the S3 bucket is hosted. Ask your instructor to verify that your data is arriving in AWS S3.
